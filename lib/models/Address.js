...
module.exports = class Address {
    ...
    static async insert({ street, name, state, city }) {
        ...
        const { rows } = await pool.query(
            'INSERT INTO addresses (street, name, state, city) VALUES ($1, $2, $3, $4) RETURNING *',
    ...
    static async updateEntry(id, street, name, state, city) {
        const { rows } = await pool.query(
            'UPDATE addresses SET street=$2, name=$3, state=$4, city=$5 WHERE id=$1 RETURNING *', [id, street, name, state, city]
        );
        return new Address(rows[0]);
    }
